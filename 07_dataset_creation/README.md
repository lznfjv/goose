# Objective
Our computer vision model that will be used aboard GooseBot must be able to detect the lines on a road and the rubber duckies (here, 'peasants') used to simulate pedestrians. By default, YOLO11 is not trained to recognize these objects. We must create a dataset that can be used to train the model to recognize these objects.
